...
function Client(urls, options) {
    ...
    options.noNetworkCache = !!(options.noNetworkCache) || false;
    ...
    Chrome(options, function (chrome) {
		...
		if (options.noNetworkCache) {
			chrome.Network.setExtraHTTPHeaders({"headers": {'Cache-Control': 'max-age=0, no-cache, no-store'}});
		}
